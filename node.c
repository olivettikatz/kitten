...
char *sounding_index = "/dev/shm/sounding.idx";
...
sounding_node sounding_node_create(char *n, sounding_callback_recieve h)
{
	...
	tmp.server_buffer_size = SOUNDING_SERVER_BUFFER_SIZE_DEFAULT;
...
sounding_node *sounding_list()
{
	...
	if (f != NULL)
	{
		...
		sounding_node *rtn = (sounding_node *)malloc(sizeof(sounding_node)*n);
		int nrtn = 0;
